...
const EditRegWorkshop = () => {
    ...
    const fetchWorkshop = async () => {
        try {
            ...
            const response = await axios.post(`${BackendAPI}/workshop/editworkshop`, { workshopId }, {
                headers: {
    ...
    const onSubmit = async (values) => {
        ...
        try {
            ...
            const response = await axios.post(`${BackendAPI}/workshop/updateworkshop`, { values, workshopId }, {
                headers: {
                    Authorization: `Bearer ${token}` // Include the token in the Authorization header
                }
            });
            ...
            SweetAlert.fire({
                ...
                if (result.dismiss === SweetAlert.DismissReason.timer) {
                    navigate(`${process.env.PUBLIC_URL}/registration/manage-reg-workshop/${layoutURL}`);
                }
            });
        } catch (error) {
    ...
    const validatename = debounce(async (value) => {
        try {
            ...
            const token = getToken();
            const response = await axios.post(`${BackendAPI}/workshop/check-workshop-name`, { dName: value }, {
                headers: {
                    Authorization: `Bearer ${token}` // Include the token in the Authorization header
                }
            });
    ...
    return (
        <Fragment>
            <Breadcrumbs parentClickHandler={handleNavigation} mainTitle="Edit Workshop" parent="Manage Workshop" title="Edit Workshop" />
                                        ...
                                        <form className='needs-validation' noValidate='' onSubmit={handleSubmit}>
                                            <Row>
                                                <Col md="4" className="mb-3">
                                                    <Field
                                                        name="dName"
                                                        initialValue={workshop.cs_workshop_name || ''} // Set initial value to workshop name, defaulting to empty string
                                                ...
                                                <Col md="4" className="mb-3">
                                                    ...
                                                    initialValue={workshop.cs_workshoptype_id || ''} 
                                                    >
                                                        {({ input, meta }) => (
                                                            <div>
                                                                <Select
...
export default EditRegWorkshop;
