...
const Eventcategories = () => {
    ...
    const fetchCategory = async () => {
        try {
            ...
            const response = await axios.get(`${BackendAPI}/eventcategory/getCategories`, {
                params: {
                    page: currentPage,
                    pageSize: pageSize,
                    search: searchText,
    ...
    const fetchCat = async () => {
        try {
            const token = getToken();
            const response = await axios.get(`${BackendAPI}/eventcategory/getCat`, {
                headers: {
    ...
    const getSettings = (item) => [
        ...
        ...(EventcategoriesPermissions?.edit === 1
            ? [
                {
                    key: '1',
                    label: (
        ...
        ...(EventcategoriesPermissions?.delete === 1
            ? [
                {
                    key: '2',
                    label: (
    ...
    const handleEdit = (catId, catName, onsite, registration) => {
        const URL = '/event/edit-event-category/';
    ...
    const handleDelete = async (catId) => {
        try {
            const token = getToken();
            await axios.delete(`${BackendAPI}/eventcategory/deletecategory/${catId}`, {
                headers: {
                    Authorization: `Bearer ${token}` // Include the token in the Authorization header
                }
            });
    ...
    const confirmStatusUpdate = async (newCat) => {
        ...
        try {
            ...
            await axios.put(`${BackendAPI}/eventcategory/UpdateStatus`, {
    ...
    const handleNavigation = () => {
        navigate(`${process.env.PUBLIC_URL}/event/add-event-category/Consoft`, { state: { prodData } });
    };
        ...
        <Fragment>
            <Breadcrumbs mainTitle={
                <>
                    ...
                    >
                        <PopoverBody>
                            From here you can create and manage categories for classifying users. <br />
                            ...
                            Default categories include User which cannot be edited or cannot be changed the status and <strong>Faculty</strong>, <strong>Delegate</strong>, and <strong>Exhibitor</strong> for which You can change the display name and <span className="text-success">Active</span> or <span className="text-danger">Inactive</span> status as needed.
                        </PopoverBody>
                    </UncontrolledPopover>
                </>
            } parent="Event App Admin" title="Manage Category" />
                                        ...
                                        <Table>
                                            <thead>
                                                <tr className='border-bottom-primary'>
                                                    ...
                                                    {EventcategoriesPermissions?.edit === 1 || EventcategoriesPermissions?.delete === 1 ? (
