...
import { required, email as validateEmail, name, expiryDate, BanImg, BackImg, Img1 } from '../Utils/validationUtils';
...
const EventAdminSetting = () => {
    ...
    const handleImageChangeforbanner = async (event, type) => {
        ...
        try {
            await BackImg(file); // Wait for the Promise to resolve
    ...
    const AdminSettingPermissions = permissions['EventAdminSetting'];

    const fetchSetting = async () => {
        try {
            const token = getToken();
            const response = await axios.get(`${BackendAPI}/eventsetting/getAdminSetting`, {
                headers: {
            ...
            if (response.data && response.data.setting) {
                ...
                const eventbackgroundsetting = settings.find(setting => setting.cs_parameter === "homepage_bg_img");
    ...
    const validatename = debounce(async (value) => {
        ...
        try {
            const token = getToken();
            const response = await axios.post(`${BackendAPI}/eventsetting/check-admin-regno`, { regstart: value }, {
                headers: {
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            const token = getToken();
            const response = await axios.post(`${BackendAPI}/eventsetting/updateAdminSettings`, formData, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
                                            ...
                                            <Row>
                                                ...
                                                <Col md="4" className="mb-3">
                                                    ...
                                                    {!selectedImageforbanner && (
                                                        <small className="form-text text-muted">
                                                            <strong>Image Size:</strong> 500KB Max <br />
                                                            <strong>Dimensions:</strong> 500(H) × 300(W) <br />
...
export default EventAdminSetting;
