...
const AddWorkshopType = () => {
    ...
    const AddWorkshopPermissions = permissions['Workshoptype'];
    ...
    const onSubmit = async (formData, event) => {
        ...
        try {
            ...
            const token = getToken();
            await axios.post(`${BackendAPI}/workshop/addworkshoptype`, formData, {
                headers: {
                    Authorization: `Bearer ${token}` // Include the token in the Authorization header
                }
            });
    ...
    const validatename = debounce(async (value) => {
        try {
            ...
            const token = getToken();
            const response = await axios.post(`${BackendAPI}/workshop/check-workshoptype-name`, { dName: value }, {
                headers: {
                    Authorization: `Bearer ${token}` // Include the token in the Authorization header
                }
            });
...
export default AddWorkshopType;
