...
import generatePDFFromBadgeListforList from '../Certificate-Design/badgeDownlode/UserListbadgeprint';
    ...
    const onSubmitSecondForm = async (values) => {
        ...
        try {
            ...
            for (const user of users) {
                ...
                try {
                    console.log('Fetching Certificate fields for category:', user.cs_reg_category);
                    const badgeFieldsData = await fetchBadgeFields(user.cs_reg_category);

                    if (!badgeFieldsData) {
                        console.error('No Certificate data found for user:', user);
                        skippedUsers.push(user);
                        continue;
                    }
            ...
            if (badgeList.length > 0) {
                ...
                toast.error('No Certificate were exported.');
            }
        ...
        <Fragment>
            <Breadcrumbs mainTitle={
                <>
                    ...
                    >
                        <PopoverBody>
                            Bulk export Certificate allows you to quickly export and manage Certificate information for many users at once in <strong>PDF</strong> format. <br />
                            You can export Certificate by category, registration type, within a range of start and end registration numbers, or by date range of user registration.
                        </PopoverBody>
                    </UncontrolledPopover>
                </>
            } parent="Certificate" title="Certificate Export" />
            <Container fluid>
                <Row>
                    <Col md="6">
                        <Card>
                                ...
                                <Form onSubmit={onSubmit}>
                                    {({ handleSubmit }) => (
                                        <form onSubmit={handleSubmit}>
                                            ...
                                            <Button color="primary" type="submit" className="mt-3 me-2">Export Certificate</Button>
                                ...
                                <Form onSubmit={onSubmitSecondForm}>
                                    {({ handleSubmit }) => (
                                        <form onSubmit={handleSubmit}>
                                            ...
                                            <Button color="primary" type="submit" className="mt-3 mr-2">Export Certificate</Button>
                                        </form>
                                    )}
                                </Form>
                            </CardBody>
