    ...
    const handleExportBadges = async () => {
        ...
        if (selectedUser) {
            ...
            try {
                ...
                toast.success(`Exporting Certificate for ${selectedUser.cs_first_name} ${selectedUser.cs_last_name}`);
            } catch (error) {
                if (error.response) {
                    // Server responded with a status other than 200 range
                    console.error('API error:', error.response.data);
                    toast.error(`Failed to export Certificate: ${error.response.data.message}`);
                } else if (error.request) {
                    // Request was made but no response received
                    console.error('Network error:', error.request);
                    toast.error('Failed to export Certificate: Network error. Please check your internet connection.');
                } else {
                    // Something else caused the error
                    console.error('Error:', error.message);
                    toast.error(`Failed to export Certificate: ${error.message}`);
                }
            }
        } else {
            toast.warning('Please select a user to Certificate the badge');
        }
    };
        ...
        <Fragment>
            <Breadcrumbs mainTitle={
                <>
                    ...
                    >
                        <PopoverBody>
                            Export a Certificate for a single user only.
                        </PopoverBody>
                    </UncontrolledPopover>
                </> 
                    ...
                    <Col sm="6">
                        <Card>
                            <CardBody>
                                ...
                                <Button color="success" onClick={handleExportBadges}>Export Certificate</Button>
                            </CardBody>
                        </Card>
                    </Col>
                </Row>
...
export default CertSingleExport;
