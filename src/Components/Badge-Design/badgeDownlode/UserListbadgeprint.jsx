...
const renderBadgeOnPage = async (pdf, badge_fields, pageHeight, isMirrored, pageWidth) => {
  try {
    ...
    for (const field of badge_fields) {
      ...
      const fontWeight = field.cs_field_weight || 'normal';
    ...
    for (const { field, containerX, containerY, containerWidth, containerHeight, fontSize, textColor, textAlign, fontType, fontWeight, rotationAngle, textContent } of originalFields) {
      ...
      // Handle text alignment within the container
      let textX = containerX; // Default to container's X position
      let mirroredTextX; // For isDouble mirrored alignment
      ...
      switch (field.cs_field_type_id) {
        ...
        case 'number':
          ...
          switch (textAlign) {
            case 'left':
              textX = containerX; // Align left
              mirroredTextX = pageWidth - (containerX + containerWidth); // Mirrored left
              break;
            case 'center':
              textX = containerX + (containerWidth / 2) - (textWidth / 2); // Align center
              mirroredTextX = pageWidth - (containerX + (containerWidth / 2) + (textWidth / 2)); // Mirrored center
              break;
            case 'right':
              textX = containerX + containerWidth - textWidth; // Align right
              mirroredTextX = pageWidth - containerX - textWidth; // Mirrored right
              break;
            default:
              console.warn(`Unknown alignment: ${textAlign}`);
              textX = containerX; // Fallback to left
              mirroredTextX = pageWidth - (containerX + containerWidth); // Fallback for mirrored left
          }

          if (isDouble) {
            // Calculate mirrored Y position
            const mirroredCenteredY = pageHeight - (containerY + containerHeight / 2) - (fontSize / 4);

            // Render mirrored text with 180-degree rotation and alignment
            pdf.text(textContent, mirroredTextX, mirroredCenteredY, { angle: rotationAngle + 180 });
          } else {
            // Handle normal text placement
            const centerY = containerY + containerHeight / 2;
            const adjustedCenterY = centerY + (fontSize / 4); // Adjust for baseline alignment

            // Render the text with rotation applied
            pdf.text(textContent, textX, adjustedCenterY, { angle: rotationAngle });
          }
          break;
        ...
        case 'image':
          ...
          // Ensure the background is white behind the image
          pdf.setFillColor(255, 255, 255); // White color
          pdf.rect(imageX, imageY, imageWidth, imageHeight, 'F'); // Fill with white
        ...
        case 'qr':
          ...
          const qrImage = await QRCode.toDataURL(qrData);
          const qrWidth = parseFloat(field.cs_field_width / 3.78);  // Adjust width if necessary
          const qrHeight = parseFloat(field.cs_field_height / 3.78); // Adjust height if necessary

          if (isDouble) {
            const rotatedQrData = await rotateImage(qrImage);
            pdf.addImage(rotatedQrData, 'PNG', containerX, containerY, qrWidth, qrHeight);
          } else {
            pdf.addImage(qrImage, 'PNG', containerX, containerY, qrWidth, qrHeight);
          }
          break;
    ...
    if (isMirrored) {
      ...
      if (backgroundImageField) {
        ...
        // console.log("mirroredY", mirroredY);
        // console.log("pageHeight", pageHeight);
        // console.log("pageWidth", pageWidth);
        // console.log("mirroredX", mirroredX);
      ...
      for (const { field, containerX, containerY, containerWidth, containerHeight, fontSize, textColor, textAlign, fontType, fontWeight, textContent } of originalFields) {
        ...
        switch (field.cs_field_type_id) {
          ...
          case 'number':
            ...
            pdf.text(textContent, mirroredCenteredX, mirroredCenteredY, { angle: 180 });
            break;
          ...
          case 'image':
            ...
            // Ensure the background is white behind the image
            pdf.setFillColor(255, 255, 255); // White color
            pdf.rect(mirroredX, mirroredY, containerWidth, containerHeight, 'F'); // Fill with white
          ...
          case 'fullname':
            ...
            // Render the text with rotation applied at the adjusted position
            pdf.text(String(field.cs_field_name), fullNameMirroredX, fullNameMirroredY, { angle: 180 });
            break;
          ...
          case 'customtext':
            ...
            pdf.text(String(field.cs_field_label), customTextMirroredX, customTextMirroredY, { angle: 180 });
            break;
